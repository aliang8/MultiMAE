...
# sys.path.append("/home/anthony/mae")
import dataset
from dataset import RealWorldData, RealWorldData
...
def get_args():
    ...
    parser.add_argument("--task_name", default="pick-place-v2", type=str)
...
def main(args):
    ...
    dataset_train = dataset.RealWorldData(
        args, dataset_path=args.data_path, transform=transform_train
    )
    ...
    if global_rank == 0 and args.log_wandb:
        # update wandb_run_name
        args.wandb_run_name = f"{args.wandb_run_name}_{args.task_name}_{args.num_encoded_tokens}_{args.in_domains}_{args.patch_size}"
    ...
    args.lr = args.blr * total_batch_size / args.batch_size
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if log_writer is not None:
            log_writer.update(
                {**{k: v for k, v in train_stats.items()}, "epoch": epoch}
            )
        if args.output_dir:
...
def train_one_epoch(
    ...
    for step, (x, saliency) in enumerate(
        ...
        # tasks_dict = {
        #     task: tensor.to(device, non_blocking=True) for task, tensor in x.items()
        # }
        tasks_dict = {
            "rgb": x.to(device, non_blocking=True),
        }

        if "saliency" in in_domains:
            tasks_dict["saliency"] = saliency.to(device, non_blocking=True)
        ...
        if standardize_depth and "depth" in tasks_dict:
            ...
            trunc_depth = torch.sort(
                rearrange(tasks_dict["depth"], "b c h w -> b (c h w)"), dim=1
            )[0]
            trunc_depth = trunc_depth[
                :, int(0.1 * trunc_depth.shape[1]) : int(0.9 * trunc_depth.shape[1])
            ]
            tasks_dict["depth"] = (
                tasks_dict["depth"] - trunc_depth.mean(dim=1)[:, None, None, None]
        ...
        with torch.cuda.amp.autocast():
            ...
            # upweight saliency loss
            # TODO: fixs this hack
            if "saliency" in weighted_task_losses:
                weighted_task_losses["saliency"] *= 10
